        ...
        if st.session_state.un_results:
            ...
            for u_idx, u in enumerate(un_list):
                with st.container(border=True):
                    ...
                    with c_utxt: st.write(f"❌ **{u['amount']:,}** ({u['info']})")
                    with c_uassign:
                    ...
                    with c_usplit:
                        if st.button("強制拆帳", key=f"u_sp_{u_idx}"):
                            task = un_list.pop(u_idx); rem, splits = task['amount'], 0
                            results.sort(key=lambda x: x['limit'], reverse=True)
                            for p in results:
                                if p['limit'] > 0 and rem > 0 and splits < 5:
                                    take = (min(rem, p['limit']) // 100 * 100) if (min(rem, p['limit']) < rem and splits < 4) else min(rem, p['limit'])
                                    if take > 0:
                                        p['tasks'].append({'info': f"{task['info']} (拆)", 'amount': int(take)})
                                        p['limit'] -= take; p['out'] += take; rem -= take; splits += 1
                            if rem > 0: un_list.insert(u_idx, {'info': task['info'], 'amount': rem})
                            st.session_state.un_results = un_list; st.session_state.current_results = results; st.rerun()
