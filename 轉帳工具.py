...
with st.sidebar:
    st.header("⚙️ 系統設定")
    buffer_val = st.slider("每人留底金額", 0, 10000, 6500, step=500)
...
with tab1:
    ...
    with btn_c1:
        ...
        if st.button("🚀 1. 執行自動分配", use_container_width=True):
            ...
            if t_list and p_list:
                ...
                st.session_state.uploaded = False # 重置上傳狀態
                st.success("✅ 系統分配完成！請在下方微調指派人。")
            else: st.error("格式錯誤")

    with btn_c2:
        # 功能二：上傳目前畫面上的結果
        if st.button("📤 2. 確認並上傳雲端", use_container_width=True, type="secondary"):
                        ...
                        for t in p['tasks']:
                            ...
                            if st.session_state.get(chk_key, False):
                                ...
                                if mask.any():
                                    idx = df[mask].index[-1]
                                    df.at[idx, '狀態'] = "完成"
                                    df.at[idx, '執行人'] = p['name']
                                    up_cnt += 1
                    if up_cnt > 0:
                        conn.update(worksheet="Sheet1", data=df)
                        st.success(f"🎯 同步成功 {up_cnt} 筆！"); st.rerun()
                    else: st.info("沒有新勾選項目。")
    ...
    if st.session_state.current_results:
        ...
        st.info(f"📊 總清單金額：{st.session_state.total_amt:,} | 已分配：{st.session_state.total_amt - un_sum:,}")
        
        results = st.session_state.current_results
        for p_idx, p in enumerate(results):
            if p['tasks']:
                with st.container(border=True):
                    st.success(f"### {p['name']} (目前預計轉出: {p['out']:,})")
                    for t_idx, tk in enumerate(p['tasks']):
                        ...
                        with c_sel:
                            new_owner = st.selectbox(
                                "換人", options=all_names_list, 
                                index=all_names_list.index(p['name']) if p['name'] in all_names_list else 0,
                                key=f"mv_{tk['info']}_{p['name']}_{tk['amount']}", label_visibility="collapsed"
                            )
                            if new_owner != p['name']:
